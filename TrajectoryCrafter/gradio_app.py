    ...
    with gr.Blocks(analytics_enabled=False, css=css) as trajcrafter_iface:
        ...
        with gr.Row():
            with gr.Row():
                ...
                i2v_show_only_generated = gr.Checkbox(
                    label="Only show generated video (without input)",
                    value=True  # Alapértelmezetten csak a generált videót mutatjuk
                )
                i2v_steps = gr.Slider(
                    minimum=1,
                    maximum=50,
        ...
        i2v_end_btn.click(
            inputs=[
                ...
                i2v_steps,
                i2v_seed,
                i2v_show_only_generated,  # Új paraméter
            ],
...
trajcrafter_iface.launch(
    server_name="0.0.0.0", server_port=12345, debug=True, share=True, max_threads=10
)
